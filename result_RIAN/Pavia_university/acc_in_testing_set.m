...
load('prob_map_180_9_199.mat') % 测试样本旋转了180度的结果
load('PaviaU_gt.mat')
...
%% GT 可视化ground truth
Label = paviaU_gt;
Label_GT_map =label2color(Label,'uni');
figure
imshow(Label_GT_map)
title('Ground Truth')
...
TrainingMap_map =label2color(TrainingMap,'uni');
figure
...
TestMap_map =label2color(TestMap,'uni');
figure
imshow(TestMap_map)
title('Testing Set')
axis image;set(gca,'xtick',[],'ytick',[]);
...
PredictMap_map =label2color(PredictMap,'uni');
figure
...
%% visual results  最终可视化结果
PredictMap =  paviaU_gt;
for i = 1:length(predict_class)
    PredictMap(TestIndex(i)) = predict_class(i);
end
PredictMap_map =label2color(PredictMap,'uni');
